...
public class EntryActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        */
        ...
        final boolean gestures = preferences.getBoolean(Constants.SETTINGS_GESTURESENABLED, true);
    ...
    private void reload() {
        ...
        if (entryCursor.moveToFirst()) {
            ...
            if (entryCursor.isNull(readDatePosition)) {
                getContentResolver().update(
                        ...
                        new StringBuilder(FeedData.EntryColumns.READDATE).append(
                                Constants.DB_ISNULL).toString(), null);
            }
            if (abstractText == null || abstractText.trim().length() == 0) {
                ...
                if (feedName == null
                        || (canShowIcon && (iconBytes == null || iconBytes.length == 0))) {
                    Cursor feedCursor = getContentResolver().query(
                            FeedData.FeedColumns.CONTENT_URI(Integer.toString(feedId)),
                            new String[] { FeedData.FeedColumns._ID, FeedData.FeedColumns.NAME,
                                    FeedData.FeedColumns.URL, FeedData.FeedColumns.ICON }, null,
                            null, null);
                    if (feedCursor.moveToFirst()) {
                        feedName = feedCursor.isNull(1) ? feedCursor.getString(2) : feedCursor
                                .getString(1);
                ...
                StringBuilder dateStringBuilder = new StringBuilder(DateFormat.getDateFormat(this)
                ...
                // html-header
                localPictures = abstractText.indexOf(Constants.IMAGEID_REPLACEMENT) > -1;

                if (localPictures) {
                    abstractText = abstractText.replace(Constants.IMAGEID_REPLACEMENT, _id
                            + Constants.IMAGEFILE_IDSEPARATOR);
                }

                if (preferences.getBoolean(Constants.SETTINGS_DISABLEPICTURES, false)) {
                    abstractText = abstractText.replaceAll(Constants.HTML_IMG_REGEX,
                            Constants.EMPTY);
                    ...
                    if (webView.getSettings().getBlockNetworkImage()) {
                         ...
                         */
                        webView.loadData(Constants.EMPTY, TEXT_HTML, UTF8);
                        webView.getSettings().setBlockNetworkImage(false);
                    }
                }

                int fontsize = Integer.parseInt(preferences.getString(Constants.SETTINGS_FONTSIZE,
                        Constants.ONE));
                ...
                /*
                if (MainTabActivity.isLightTheme(this)
                        || preferences.getBoolean(Constants.SETTINGS_BLACKTEXTONWHITE, false)) {
                    if (fontsize > 0) {
                        webView.loadDataWithBaseURL(
                                null,
                                new StringBuilder(CSS).append(FONTSIZE_START).append(fontsize)
                                        .append(FONTSIZE_MIDDLE).append(abstractText)
                                        .append(FONTSIZE_END).toString(), TEXT_HTML, UTF8, null);
                    } else {
                        webView.loadDataWithBaseURL(null, new StringBuilder(CSS).append(BODY_START)
                                .append(abstractText).append(BODY_END).toString(), TEXT_HTML, UTF8,
                                null);
                    }
                    ...
                    if (fontsize > 0) {
                        webView.loadDataWithBaseURL(null, new StringBuilder(FONT_FONTSIZE_START)
                                .append(fontsize).append(FONTSIZE_MIDDLE).append(abstractText)
                                .append(FONT_END).toString(), TEXT_HTML, UTF8, null);
                    } else {
                        webView.loadDataWithBaseURL(null,
                                new StringBuilder(FONT_START).append(abstractText).append(BODY_END)
                ...
                */
                ...
                if (link != null && link.length() > 0) {
                    ...
                    urlButton.setOnClickListener(new OnClickListener() {
                        public void onClick(View view) {
                            startActivityForResult(new Intent(Intent.ACTION_VIEW, Uri.parse(link)),
                                    0);
                        }
                    });
                } else {
                ...
                if (enclosure != null && enclosure.length() > 6
                    ...
                    playButton.setOnClickListener(new OnClickListener() {
                        public void onClick(View v) {
                            ...
                            final int position2 = enclosure.indexOf(Constants.ENCLOSURE_SEPARATOR,
                                    position1 + 3);

                            final Uri uri = Uri.parse(enclosure.substring(0, position1));

                            if (preferences.getBoolean(Constants.SETTINGS_ENCLOSUREWARNINGSENABLED,
                                    true)) {
                                ...
                                if (position2 + 4 > enclosure.length()) {
                                    builder.setMessage(getString(
                                            ...
                                            uri,
                                            position2 + 4 > enclosure.length() ? Constants.QUESTIONMARKS
                                        ...
                                        new DialogInterface.OnClickListener() {
                                            public void onClick(DialogInterface dialog, int which) {
                                                preferences
                                                        .edit()
                                                        .putBoolean(
                                                                Constants.SETTINGS_ENCLOSUREWARNINGSENABLED,
                                                                false).commit();
                                                showEnclosure(uri, enclosure, position1, position2);
                                            }
                                        });
    ...
    private void showEnclosure(Uri uri, String enclosure, int position1, int position2) {
        try {
            ...
            try {
                ...
                // fallbackmode - let the browser handle this
            } catch (Throwable t) {
    ...
    private void setupButton(ImageButton button, final boolean successor, long date) {
        StringBuilder queryString = new StringBuilder(DATE).append(date).append(AND_ID)
                .append(successor ? '>' : '<').append(_id).append(')').append(OR_DATE)
                .append(successor ? '<' : '>').append(date);

        if (hideRead) {
            queryString.append(Constants.DB_AND).append(EntriesListAdapter.READDATEISNULL);
        }

        Cursor cursor = getContentResolver().query(parentUri,
                new String[] { FeedData.EntryColumns._ID }, queryString.toString(), null,
                successor ? DESC : ASC);
    ...
    private void showShare(String shareString) {
        ...
        oks.setSiteUrl("http://www.imlongluo.com");
