...
public class RefreshService extends Service {
    ...
    private OnSharedPreferenceChangeListener listener = new OnSharedPreferenceChangeListener() {
        public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {
            if (Constants.SETTINGS_REFRESHINTERVAL.equals(key)) {
                restartTimer(false);
            }
        }
    };
    ...
    public void onCreate() {
        ...
        refreshBroadcastIntent = new Intent(Constants.ACTION_REFRESHFEEDS).putExtra(
                Constants.SCHEDULED, true);
    ...
    private void restartTimer(boolean created) {
        ...
        try {
            time = Math.max(60000, Integer.parseInt(preferences.getString(
                    Constants.SETTINGS_REFRESHINTERVAL, SIXTYMINUTES)));
        ...
        if (created) {
            long lastRefresh = preferences.getLong(Constants.PREFERENCE_LASTSCHEDULEDREFRESH, 0);

            if (lastRefresh > 0) {
                // this indicates a service restart by the system
                initialRefreshTime = Math.max(SystemClock.elapsedRealtime() + 10000, lastRefresh
                        + time);
            }
        }

        alarmManager.setInexactRepeating(AlarmManager.ELAPSED_REALTIME_WAKEUP, initialRefreshTime,
                time, timerIntent);
    }
