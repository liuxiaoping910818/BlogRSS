		...
		if (cursor.moveToFirst()) {
			...
			findViewById(R.id.save_button).setOnClickListener(new OnClickListener() {
				public void onClick(View view) {
					SharedPreferences.Editor preferences = getSharedPreferences(BlogReaderAppWidgetProvider.class.getName(), 0).edit();
					...
					int color = getPreferenceManager().getSharedPreferences().getInt("widget.background", BlogReaderAppWidgetProvider.STANDARD_BACKGROUND);

					preferences.putInt(widgetId+".background", color);
					preferences.commit();

					BlogReaderAppWidgetProvider.updateAppWidget(WidgetConfigActivity.this, widgetId, hideRead, entryCount, feedIds, color);
